...
class DQN():
	...
	def train_Q_network(self):
		...
		target_Q_value_batch = self.target_Q_value.eval(feed_dict={self.state_input:next_state_batch})
		for i in range(0,BATCH_SIZE):
			done = minibatch[i][4]
			if done:
				y_batch.append(reward_batch[i])
			else :
				Q_value=target_Q_value_batch[i,max_action_next[i]]
				y_batch.append(reward_batch[i] + GAMMA*Q_value)
